...
export class GuidersClient {
	...
	private static async fetchTokens(): Promise<{ access_token: string; refresh_token: string } | null> {
		try {
			const response = await fetch(`${this.options.url}/auth/token`, {
				method: "POST",
	...
	private static async refreshAccessToken(): Promise<boolean> {
		try {
			const response = await fetch(`${this.options.url}/auth/refresh`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ apiKey: this.options.apiKey, refreshToken: this.refreshToken }),
			});
